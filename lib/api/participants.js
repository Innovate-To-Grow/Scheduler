export async function fetchParticipants(code) {
  const res = await fetch(`/api/events/participants?code=${encodeURIComponent(code)}`);
...
export async function updateParticipant(code, name, data) {
  const res = await fetch(
    `/api/events/participants/update?code=${encodeURIComponent(code)}&name=${encodeURIComponent(name)}`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
